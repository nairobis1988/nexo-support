...
export const SOCIAL_CARD_CONTENT_HEIGHT = 1776;
export const SOCIAL_CARD_LOGO_SIZE = 104;
export const SOCIAL_CARD_LOGO_LEFT = 932;
export const SOCIAL_CARD_LOGO_TOP = 1796;
...
async function normalizedVerticalPreview(image) {
  ...
  try {
    const mask = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="${SOCIAL_CARD_CONTENT_HEIGHT}">
      <path d="M72 0 H1008 Q1080 0 1080 72 V${SOCIAL_CARD_CONTENT_HEIGHT} H0 V72 Q0 0 72 0 Z" fill="white"/>
    </svg>`);
    return await sharp(image.bytes)
      .rotate()
...
export function buildVerticalShareCardSvg({ type, authorName, caption, hasPreview, hasAvatar }) {
  ...
  const play = isVideo
    ? `<circle cx="540" cy="888" r="76" fill="#010305" fill-opacity="0.70" stroke="#fff" stroke-opacity="0.48" stroke-width="2"/>
       <circle cx="540" cy="888" r="67" fill="#111820" fill-opacity="0.28"/>
       <path d="M520 842 L520 934 L588 888 Z" fill="#fff"/>`
    : '';
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1920">
    ...
    <path d="M5 ${SOCIAL_CARD_CONTENT_HEIGHT} H1075 V1847 Q1075 1915 1007 1915 H73 Q5 1915 5 1847 Z" fill="url(#footer)"/>
    <rect x="48" y="${SOCIAL_CARD_CONTENT_HEIGHT}" width="984" height="2" fill="url(#separator)"/>
    ${play}
    ${border}
  </svg>`;
}
...
export async function renderVerticalShareCard({ type, authorName, caption, preview, avatar, logoBytes }) {
  ...
  const logoSize = previewBytes ? SOCIAL_CARD_LOGO_SIZE : SOCIAL_CARD_FALLBACK_LOGO_SIZE;
  ...
  layers.push({
    input: logo,
    left: previewBytes ? SOCIAL_CARD_LOGO_LEFT : SOCIAL_CARD_FALLBACK_LOGO_LEFT,
    top: previewBytes ? SOCIAL_CARD_LOGO_TOP : SOCIAL_CARD_FALLBACK_LOGO_TOP,
  });
