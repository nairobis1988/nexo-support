...
async function readFirstStorageImage(client, candidates) {
...
export async function fetchSocialCardData({
  type,
  id,
  request,
  oidcTokenReader = readVercelOidcToken,
  authSessionFactory = createGcpAuthSession,
  documentReader = readDocument,
  assetReader = readFirstStorageImage,
}) {
  if (!isValidContentType(type) || !isValidContentId(id)) return null;
  ...
  let avatar;
  try {
    [preview, avatar] = await Promise.all([
      assetReader(client, mediaCandidates(type, content, author, 'content')),
      assetReader(client, mediaCandidates(type, content, author, 'avatar')),
    ]);
  } catch {
    throw new Error('storage_unavailable');
  }
