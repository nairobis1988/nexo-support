...
export async function fetchPublicContent({
  ...
  authClientFactory = createGcpAuthClient,
  ...
  const client = authClientFactory(oidcToken);
  diagnose(diagnosticLog, 'auth_client_created', type, 'created');
  const collection = firestoreCollectionForType(type);
  diagnose(diagnosticLog, 'firestore_content_request_started', type, 'started');
  const content = await documentReader(client, collection, id);
  if (!content) {
    diagnose(diagnosticLog, 'firestore_content_404', type, 'not_found');
    return null;
  }
  diagnose(diagnosticLog, 'firestore_content_found', type, 'found');
  ...
  diagnose(diagnosticLog, 'author_request_started', type, 'started');
  const author = await documentReader(client, 'users', ownerId);
  if (author) {
    diagnose(diagnosticLog, 'author_found', type, 'found');
  } else {
    diagnose(diagnosticLog, 'author_404', type, 'not_found');
  }
